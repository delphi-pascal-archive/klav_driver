...
beep proc near
 ...
 out 61h,al
 mov ax,1000
 ; Учтановить частоту звука
 ; таймер начинает действовать немедленно
 ; по засылке счетчика
 out 42h,al
 mov al,ah
 out 42h,al
 ...
 pop cx
 loop LOO
 ; Отключить какнал от динамика (прекратить звук)
 in al,61h
 and al,11111100B
 out 61h,al
 pop ax
...
simbol proc near	; вывод 'beep' после 'u+символ'
 ...
 inc dl
 inc dl
 xor al,'u'		; символ перед предыдущим 'u'?
 jnz back		; нет - возврат
...
; Команда 0 Инициализация
 ...
 initialization:
 call initial 			; Вывод начального сообщения
...
ret
initial endp
msg1 db '<<< Console driver [by Simagin Andrey, IT-4 (2)] >>>',0dh,0ah,'$'
